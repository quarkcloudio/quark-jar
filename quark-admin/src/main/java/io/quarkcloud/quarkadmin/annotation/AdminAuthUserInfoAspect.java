...
@Aspect
@Component
public class AdminAuthUserInfoAspect {

    // 加载基础资源包路径
    public String[] basePackages = Config.getInstance().getBasePackages("admin");

    @Pointcut("@annotation(io.quarkcloud.quarkadmin.annotation.AdminAuthUserInfo)")
    private void AdminAuthUserInfo() {}

    /**
     * 环绕通知
     */
    @Around("AdminAuthUserInfo()")
    public Object advice(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        for (Class<?> clazz : classes) {
            if(clazz.getSimpleName().equals(resource)) {
                result = new ClassLoader().setClazz(clazz).doMethod("userInfo", newContext);
            }
        }
