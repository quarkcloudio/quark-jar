...
@Aspect
@Component
public class AdminResourceCreateRenderAspect {

    // 加载基础资源包路径
    public String[] basePackages = Config.getInstance().getBasePackages("admin");

    @Pointcut("@annotation(io.quarkcloud.quarkadmin.annotation.AdminResourceCreateRender)")
    private void AdminResourceCreateRender() {}

    /**
     * 环绕通知
     */
    @Around("AdminResourceCreateRender()")
    public Object advice(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 得到方法上的注解
        AdminResourceCreateRender annotation = method.getAnnotation(AdminResourceCreateRender.class);
        if (annotation==null) {
        ...
        for (Class<?> clazz : classes) {
            if(clazz.getSimpleName().equals(resource)) {
                result = new ClassLoader().setClazz(clazz).doMethod("creationRender", newContext);
            }
        }
