...
@Aspect
@Component
public class AdminResourceIndexRenderAspect {

    // 加载基础资源包路径
    public String[] basePackages = Config.getInstance().getBasePackages("admin");

    @Pointcut("@annotation(io.quarkcloud.quarkadmin.annotation.AdminResourceIndexRender)")
    private void AdminResourceIndexRender() {}

    /**
     * 环绕通知
     */
    @Around("AdminResourceIndexRender()")
    public Object advice(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 得到方法上的注解
        AdminResourceIndexRender annotation = method.getAnnotation(AdminResourceIndexRender.class);
        if (annotation==null) {
        ...
        for (Class<?> clazz : classes) {
            if(clazz.getSimpleName().equals(resource)) {
                result = new ClassLoader().setClazz(clazz).doMethod("indexRender", newContext);
            }
        }
