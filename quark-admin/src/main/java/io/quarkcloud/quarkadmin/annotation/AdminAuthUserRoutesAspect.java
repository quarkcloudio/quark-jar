...
@Aspect
@Component
public class AdminAuthUserRoutesAspect {

    // 加载基础资源包路径
    public String[] basePackages = Config.getInstance().getBasePackages("admin");

    @Pointcut("@annotation(io.quarkcloud.quarkadmin.annotation.AdminAuthUserRoutes)")
    private void AdminAuthUserRoutes() {}

    /**
     * 环绕通知
     */
    @Around("AdminAuthUserRoutes()")
    public Object advice(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 得到方法上的注解
        AdminAuthUserRoutes annotation = method.getAnnotation(AdminAuthUserRoutes.class);
        if (annotation==null) {
        ...
        for (Class<?> clazz : classes) {
            if(clazz.getSimpleName().equals(resource)) {
                result = new ClassLoader().setClazz(clazz).doMethod("userRoutes", newContext);
            }
        }
