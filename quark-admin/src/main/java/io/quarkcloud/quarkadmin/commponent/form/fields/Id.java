...
public class Id extends Commponent {
    ...
    boolean onIndexDisplayed;

    boolean onDetailDisplayed;

    boolean onFormDisplayed;

    boolean onExportDisplayed;
    ...
    // });
    public Id setCreationRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    // rule.unique("admins", "username", "用户名已存在"),
    // });
    public Id setUpdateRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    // 生成前端验证规则
    public Id buildFrontendRules(String path) {
        Rule[] rules = new Rule[] {};
    ...
    // 表头的筛选菜单项，当值为 true 时，自动使用 valueEnum 生成，只在列表页中有效
    public Id setFilters(boolean filters) {
    ...
    // 表头的筛选菜单项，当值为 true 时，自动使用 valueEnum 生成，只在列表页中有效
    public Id setFilters(Map<String, String> filters) {
    ...
    // 设置When组件数据
    //
    // new Id().setWhen(option, callback)
    public Id setWhen(Object option, Closure callback) {
    ...
    //
    // new Id().setWhen(">", option, callback)
    public Id setWhen(String operator, Object option, Closure callback) {
        When w = new When();
    ...
    public Id hideFromIndex(boolean callback) {
    ...
    public Id hideFromDetail(boolean callback) {
    ...
    public Id hideWhenCreating(boolean callback) {
    ...
    public Id hideWhenUpdating(boolean callback) {
    ...
    public Id hideWhenExporting(boolean callback) {
    ...
    public Id hideWhenImporting(boolean callback) {
    ...
    // Specify that the element should be hidden from the index view.
    public Id onIndexShowing(boolean callback) {
        this.showOnIndex = callback;
    ...
    // Specify that the element should be hidden from the detail view.
    public Id onDetailShowing(boolean callback) {
        this.showOnDetail = callback;
    ...
    // Specify that the element should be hidden from the creation view.
    public Id showOnCreating(boolean callback) {
        this.showOnCreation = callback;
    ...
    // Specify that the element should be hidden from the update view.
    public Id showOnUpdating(boolean callback) {
        this.showOnUpdate = callback;
    ...
    // Specify that the element should be hidden from the export file.
    public Id showOnExporting(boolean callback) {
        this.showOnExport = callback;
    ...
    // Specify that the element should be hidden from the import file.
    public Id showOnImporting(boolean callback) {
        this.showOnImport = callback;
