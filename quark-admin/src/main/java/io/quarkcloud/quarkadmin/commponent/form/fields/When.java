...
public class When extends Commponent {
    ...
    public When setWidth(Object width) {
    ...
    //	}
    public When setRules(Rule[] rules) {
    ...
    //	}
    public When setCreationRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    //	}
    public When setUpdateRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    // 生成前端验证规则
    public When buildFrontendRules(String path) {
        Rule[] rules = new Rule[]{};
    ...
    // 表头的筛选菜单项，当值为 true 时，自动使用 valueEnum 生成，只在列表页中有效
    public When setFilters(boolean filters) {
    ...
    // 表头的筛选菜单项，当值为 true 时，自动使用 valueEnum 生成，只在列表页中有效
    public When setFilters(Map<String, String> filters) {
