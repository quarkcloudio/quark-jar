...
public class SwitchField extends Commponent {
    ...
    // });
    public SwitchField setRules(Rule[] rules) {
    ...
    // });
    public SwitchField setCreationRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    // rule.unique("admins", "username", "用户名已存在"),
    // });
    public SwitchField setUpdateRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
