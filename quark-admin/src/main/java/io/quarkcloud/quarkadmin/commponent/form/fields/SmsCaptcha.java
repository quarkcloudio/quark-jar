...
public class SmsCaptcha extends Commponent {
    ...
    // });
    public SmsCaptcha setRules(Rule[] rules) {
    ...
    // });
    public SmsCaptcha setCreationRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
    ...
    // rule.unique("admins", "username", "用户名已存在"),
    // });
    public SmsCaptcha setUpdateRules(Rule[] rules) {
        for (int i = 0; i < rules.length; i++) {
            rules[i] = rules[i].setName(name);
        }
